...
def settingsFileCreation(settingsFile):
    if not os.path.exists(settingsFile):
        with open(settingsFile, "w") as file:
            print(f"File '{settingsFile}' created and 'Debloated' written.")
 

def mainMenu():
    global a,b,cskinDir,useDebloat, useDebloat2x, useDebloatA
...
def copyFiles():
    ...
    if os.path.exists(copied_skin_dir):
        ...
        if d == "y":
            ...
            print("Removed Old Debloated Folder")
            copyFunction()
        else:
